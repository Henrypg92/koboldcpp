...
def init_library():
    """
    Inicia la biblioteca dinámica y configura las opciones de aceleración.

    Variables globales utilizadas:
    - handle: Manejador de la biblioteca dinámica
    - args: Argumentos de la aplicación
    - lib_default, lib_failsafe, lib_openblas, lib_noavx2, lib_clblast, lib_clblast_noavx2, lib_cublas, lib_hipblas: Rutas de las bibliotecas

    Este método detecta la presencia de bibliotecas específicas para aceleración y configura las opciones correspondientes.
    """
    # Declarar variables globales
    global handle, args
    global lib_default, lib_failsafe, lib_openblas, lib_noavx2, lib_clblast, lib_clblast_noavx2, lib_cublas, lib_hipblas

    # Inicializar nombre de la librería y banderas de aceleración
    libname = ""
    use_openblas = False  # si es verdadero, utiliza OpenBLAS para aceleración. libopenblas.dll debe estar en el mismo directorio.
    use_clblast = False    # utiliza CLBlast en su lugar
    use_cublas = False     # utiliza cublas en su lugar
    use_hipblas = False    # utiliza hipblas en su lugar
    use_noavx2 = False     # utiliza instrucciones no avx2
    use_failsafe = False   # utiliza sin instrucciones intrínsecas, modo seguro

    # Verificar argumentos de la línea de comandos para establecer las banderas de aceleración
    if args.noavx2:
        use_noavx2 = True
        if args.useclblast:
            # Verificar si existe el archivo de la biblioteca NoAVX2 CLBlast
            if not file_exists(lib_clblast_noavx2) or (os.name == 'nt' and not file_exists("clblast.dll")):
                print("Advertencia: No se encontró el archivo de la biblioteca NoAVX2 CLBlast. Se utilizará una biblioteca no-BLAS.")
            else:
                print("Intentando usar la biblioteca NoAVX2 CLBlast para una ingestión de texto más rápida. Se requerirá un clblast compatible.")
                use_clblast = True
        else:
            # Verificar si existe el archivo de la biblioteca NoAVX2
            if not file_exists(lib_noavx2):
                print("Advertencia: No se encontró el archivo de la biblioteca NoAVX2. Se utilizará una biblioteca de modo seguro.")
            elif (args.noblas and args.nommap):
                use_failsafe = True
                print("!!! Intentando usar el MODO SEGURO !!!")
            else:
                print("Intentando usar una biblioteca de compatibilidad sin avx2.")
    elif args.useclblast:
        # Verificar si existe el archivo de la biblioteca CLBlast
        if not file_exists(lib_clblast) or (os.name == 'nt' and not file_exists("clblast.dll")):
            print("Advertencia: No se encontró el archivo de la biblioteca CLBlast. Se utilizará una biblioteca no-BLAS.")
        else:
            print("Intentando usar la biblioteca CLBlast para una ingestión de texto más rápida. Se requerirá un clblast compatible.")
            use_clblast = True
    elif (args.usecublas is not None):
        # Verificar si existe el archivo de la biblioteca CuBLAS o hipBLAS
        if not file_exists(lib_cublas) and not file_exists(lib_hipblas):
            print("Advertencia: No se encontró el archivo de la biblioteca CuBLAS. Se utilizará una biblioteca no-BLAS.")
        else:
            if file_exists(lib_cublas):
                print("Intentando usar la biblioteca CuBLAS para una ingestión de texto más rápida. Se requerirá un CuBLAS compatible.")
                use_cublas = True
            elif file_exists(lib_hipblas):
                print("Intentando usar la biblioteca hipBLAS para una ingestión de texto más rápida. Se requerirá una GPU AMD compatible.")
                use_hipblas = True
    else:
        # Verificar si existe el archivo de la biblioteca OpenBLAS
        if not file_exists(lib_openblas) or (os.name == 'nt' and not file_exists("libopenblas.dll")):
            print("Advertencia: No se encontró el archivo de la biblioteca OpenBLAS. Se utilizará una biblioteca no-BLAS.")
        elif args.noblas:
            print("Intentando usar la biblioteca sin OpenBLAS.")
        else:
            use_openblas = True
            print("Intentando usar la biblioteca OpenBLAS para una ingestión de texto más rápida. Se requerirá un libopenblas compatible.")
            if sys.platform == "darwin":
                print("Nota para Mac OSX: Algunas personas han encontrado que Accelerate es en realidad más rápido que OpenBLAS. Para comparar, ejecute Koboldcpp con --noblas en su lugar.")

    # Establecer el nombre de la biblioteca según las opciones elegidas
    if use_noavx2:
        if use_failsafe:
    ...
    # Imprimir la biblioteca seleccionada para la inicialización
    print("Inicializando la biblioteca dinámica: " + libname)
    dir_path = getdirpath()
    abs_path = getabspath()

    # Agregar rutas potenciales para bibliotecas dinámicas (DLL en Windows)
    if os.name == 'nt':
        ...
        if libname == lib_hipblas and "HIP_PATH" in os.environ:
            ...
            if args.debugmode == 1:
                print(f"La ruta del SDK HIP/ROCm en {os.environ['HIP_PATH']} se incluyó en la carga de .DLL")

    # Cargar la biblioteca dinámica usando ctypes
    handle = ctypes.CDLL(os.path.join(dir_path, libname))

    # Establecer tipos de argumentos y tipos de retorno para varias funciones en la biblioteca
    handle.load_model.argtypes = [load_model_inputs]
    handle.load_model.restype = ctypes.c_bool
    handle.generate.argtypes = [generation_inputs, ctypes.c_wchar_p]  # aparentemente necesario para que funcione en osx
...
def load_model(model_filename):
    ...
    Retorna:
    - Valor booleano indicando si la carga del modelo fue exitosa
    """
    global args  # Se refiere a una variable global llamada args

    # Configuración de las entradas para cargar el modelo
    inputs = load_model_inputs()  # Crea un objeto inputs utilizando una función load_model_inputs()
    inputs.model_filename = model_filename.encode("UTF-8")  # Convierte el nombre del archivo a formato UTF-8
    inputs.max_context_length = maxctx  # Valor inicial a usar para ctx, puede ser sobrescrito
    inputs.threads = args.threads
    inputs.low_vram = (True if (args.usecublas and "lowvram" in args.usecublas) else False)  # Configura low_vram
    inputs.use_mmq = (True if (args.usecublas and "mmq" in args.usecublas) else False)  # Configura use_mmq
    inputs.blasthreads = args.blasthreads
    inputs.use_mmap = (not args.nommap)  # Configura use_mmap
    inputs.use_mlock = args.usemlock
    inputs.lora_filename = "".encode("UTF-8")  # Inicializa lora_filename como una cadena vacía en formato UTF-8
    inputs.lora_base = "".encode("UTF-8")  # Inicializa lora_base como una cadena vacía en formato UTF-8
    
    # Configuración de lora_filename y lora_base si se proporciona la opción --lora
    if args.lora:
    ...
    inputs.use_contextshift = (0 if args.noshift else 1)  # Configura use_contextshift basado en la opción --noshift
    inputs.blasbatchsize = args.blasbatchsize
    inputs.forceversion = args.forceversion
    inputs.gpulayers = args.gpulayers
    inputs.rope_freq_scale = args.ropeconfig[0]
    
    # Configuración de rope_freq_base si se proporciona más de un valor en --ropeconfig
    ...
    # Configuración de tensor_split basado en la opción --tensor_split
    ...
    # Debemos forzar una división explícita del tensor para evitar problemas multigpu
    inputs.cublas_info = 0

    # Configuración de CUDA_VISIBLE_DEVICES y HIP_VISIBLE_DEVICES basado en la opción --usecublas
    ...
    inputs.executable_path = (getdirpath() + "/").encode("UTF-8")  # Configura la ruta del ejecutable en formato UTF-8
    inputs.debugmode = args.debugmode  # Configura el modo de depuración
    banned_tokens = args.bantokens
    
    # Configuración de banned_tokens en inputs.banned_tokens
    ...
    ret = handle.load_model(inputs)  # Llama a la función load_model con las configuraciones proporcionadas
    return ret  # Retorna el valor de la carga del modelo (booleano)
